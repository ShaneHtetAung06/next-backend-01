...
import { X_HEADER_USER_EMAIL, X_HEADER_USER_ID, X_HEADER_USER_NAME } from "./lib/constant";
...
export function middleware(request) {
  ...
  if (request.method === "OPTIONS") {
    return NextResponse.json({}, { status: 200, headers: dynamicCorsHeaders });
  }
  ...
  if (pathname.startsWith("/api/item") || pathname.startsWith("/api/user")) {
    ...
    if (!user) {
      return NextResponse.json({ message: "Unauthorized Request" }, { status: 401, headers: dynamicCorsHeaders });
    }
